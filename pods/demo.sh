...
run "kubectl -n demos get pod pods-demo-pod -o custom-columns=.NAME:status.podIP --no-headers"

trap "" SIGINT
IP=$( kubectl -n demos get pod pods-demo-pod -o custom-columns=.NAME:status.podIP --no-headers )
...
run "kubectl -n demos delete pod pods-demo-pod"
